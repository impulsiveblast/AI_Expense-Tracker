...
export const generateMonthlyInsight = async ({
    ...
    // FIX 1: The prompt template literal was broken — closing backtick was misplaced.
    // The JSON schema and constraints must be INSIDE the same template literal.
...
// FIX 2: generateSavingsTips was missing its try/catch and API call entirely,
// and its closing brace was absent — causing analyzeTransactionList to be nested inside it.
...
Provide 3 tips. Reference actual numbers. Tone: encouraging and practical.`;
...
export const analyzeBudgetList = async ({ budgets, currency = 'USD' }) => {
    ...
    const prompt = `You're a personal finance assistant. Analyze each budget below and provide a one-sentence validation.
...
- status: 'good' (well-paced, under target), 'caution' (approaching limit or above 70%), or 'concerning' (over limit)
